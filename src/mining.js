...
async function selectPool(minerData, algo) {
 ...
 //   fetch(`https://raw.githubusercontent.com/RuhmitSahu/e/${isDev ? "dev" : "main"}/pools.json`)
		.then(res => res.json())
		.then(async poolData => {
			spinner.stop();
			const poolList = [];
		 ...
		 pool = await inquirer.prompt({
			...
			var regionList = [];
			if(algo == "etchash" || algo == "kawpow" || algo == "beamv3" || algo == "EQUI144_5" || algo == "randomx"){
				regionList = [];
			} else {
			//	console.log(chalk.green(`AAAA ${poolList[1].name}`))
			console.log(chalk.bold.cyan(`Ignore "us2" on NiceHash lol -Plerion`))
				regionList = ["us2"];
			}
			// if(poolList[0].name == "Ethermine") {
			// 	regionList = ["us2"];
			// }
